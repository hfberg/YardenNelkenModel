...
E_act  = zeros(P,NE,floor(t_eq/dt)); % Activity of all excitatory neurons 
%during the time allowed for reaching equilibrium, for identification of
%the active neurons. %This generates a 11 x 100 x50000  matrix (if P is set
%to 11) /HFB
...
sigma = 0.01; %choosing the spread of the distribution
...
gauss_array = zeros(1,50000); %initializing an array to display a histogram of the U-values used in the loop /HFB
for i = 1:floor(t_eq/dt)
    ...
    gauss_array(i) = gauss_array(i) + U; %adding each U to the array to be displayd in a histogram.
...
Neuron_mean    = mean(E_act,3); %Calculates the mean of the activity of the same neuron in time.

Active_Neurons = Neuron_mean > act_thresh; %The activity threshold is set to zero, whiwh means that only neurons with input in Hz> 0 will be used.
...
save(filename) 
save([foldnam '/TYLT_filename_' term '.mat'],'filename')
